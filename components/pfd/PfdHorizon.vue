<template>
  <div class="relative rounded-3xl overflow-hidden" :style="{ width: `${width}px`, height: `${height}px` }">
    <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white">
      <div :style="{ transform: `rotate(${bank}deg)`, transition: 'transform 0.1s ease-in-out' }">
        <div :style="{ transform: `translateY(${pitch * height / 50}px)`, transition: 'transform 0.1s ease-in-out' }">
          <div class="relative bg-blue-700 border-b border-white" :style="{ width: `${diagonal}px`, height: `${height * 180/50}px` }">
        ...
        <span class="absolute top-0 left-1/2 transform text-3xl" :style="{ transform: `translate(-50%, ${height * 160/50}px)` }">△</span>
      </div>
